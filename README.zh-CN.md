...
默认镜像现在是最小化方案，只包含 `cc-connect` 和 Codex CLI。卷挂载、宿主机 UID/GID 映射、配置路径、Webhook 端口暴露，以及需要 `ffmpeg` 或其他 agent CLI 时的派生镜像示例，见 [docs/docker-compose.md](docs/docker-compose.md)。
